...
export const FilePreview: React.FC<FilePreviewProps> = ({ node }) => {
  if (!node) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-[#050814] text-slate-500 font-mono text-xs p-6 select-none">
        <div>[No file loaded or file buffer unavailable]</div>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-[#050814] text-slate-200 font-mono text-xs select-none">
      ...
      <div className="bg-[#0b1024] p-2 border-b border-slate-800 flex items-center justify-between text-[11px] text-slate-400">
        <div className="flex items-center space-x-2 truncate">
          <span className="text-cyan-300 font-bold truncate">{node.name || 'Untitled'}</span>
          <span>({node.size || 0} bytes)</span>
        </div>
        <span className="uppercase text-slate-500">{node.type || 'UNKNOWN'}</span>
      </div>
